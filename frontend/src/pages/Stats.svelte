<script lang="ts">
  ...
  let topVulnTypes: { name: string; count: number }[] = [];
  let verifiedPct = 0;
  ...
  function computeStats() {
    ...
    topVulnTypes = [
      { name: 'Critical', count: critTotal },
      { name: 'High', count: highTotal },
      { name: 'Medium', count: medTotal },
      { name: 'Low', count: lowTotal },
    ].filter(v => v.count > 0);
  }
</script>

<div class="max-w-6xl mx-auto px-4 py-8">
  <h1 class="text-2xl font-bold mb-1">BSC Security Health</h1>
  <p class="text-sm text-[var(--text-secondary)] mb-8">Aggregated security metrics across {totalScanned} audited contracts</p>
  ...
  {:else if totalScanned === 0}
    <div class="text-center py-12 text-[var(--text-secondary)]">No data yet. Scan some contracts first.</div>
  {:else}
    <!-- Overview cards -->
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      <div class="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 text-center">
        <div class="text-3xl font-bold">{totalScanned}</div>
        <div class="text-xs text-[var(--text-secondary)] mt-1">Contracts Scanned</div>
      </div>
      <div class="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 text-center">
        <div class="text-3xl font-bold" style="color: {scoreBgColor(avgScore)}">{avgScore}</div>
    ...
    <!-- Distribution -->
    <div class="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6 mb-8">
          ...
          <div>
            ...
            <div class="h-2 bg-[var(--bg-primary)] rounded-full overflow-hidden">
              <div
                ...
                style="width: {totalScanned > 0 ? (bar.count / totalScanned) * 100 : 0}%; background: {bar.color}"
              ></div>
            </div>
          </div>
        {/each}
    ...
    {#if topVulnTypes.length > 0}
      <div class="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6">
        <h3 class="text-sm font-semibold mb-4 m-0">Findings by Severity</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          {#each topVulnTypes as vuln}
            <div class="text-center">
              <div class="text-2xl font-bold">{vuln.count}</div>
              <div class="text-xs text-[var(--text-secondary)]">{vuln.name}</div>
            </div>
          {/each}
        </div>
      </div>
